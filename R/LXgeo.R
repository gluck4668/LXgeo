...
LXgeo <- function(GSE_id,Platforms){
  ...
  down_file <- function(GSE_id){
    ...
    str0=strsplit(html,split = paste0("href=\"",GSE_id))[[1]]  #查看网址，找出GSE_id文件前后的特殊字符
    
    n=length(str0) # 查看有多少个文件(n-1)
    
    file_table <- data.frame(str1=rep(NA,n-1),str2=rep(NA,n-1),file_name=rep(NA,n-1))
    
    i=1
    while (i<n)
    {
      file_table[i,1]=strsplit(html,split = paste0("href=\"",GSE_id))[[1]][i+1]
      file_table[i,2] <- strsplit(file_table[i,1],split = paste0("\">",GSE_id))[[1]][1]
      file_table[i,3] <- paste0(GSE_id,file_table[i,2])
      
      url=paste0(website,file_table[i,3])
      file_path <- paste0("analysis results/(Supplementary data) ",basename(url))
      if (!file.exists(file_path)){
        print(paste0("Downloading the Series supplementary data of ",file_table[i,3]," ......" ))
        download.file(url,file_path,method = "libcurl",quiet = F ,mode = "wb")} else
          print( paste0("(Supplementary data) ",file_table[i,3]," already exists and does not need to be downloaded again.") )
...
LXlimma_02 <- function(gene_matrix){
  ...
  vol_file <- paste0("analysis results/",GSE_id,"-",Platforms," DEGs data (",cts,")_Definition.xlsx")
  ...
  p1_file <- paste0("analysis results/",GSE_id,"-",Platforms," Gene Volcano graphics (",cts,")_Definition.png")
